/*
   CYSLI RF95 RADIO PROGRAM (PAYLOAD-SERVO/GPS)
...
void loop() {
  ...
  if (rf95.available()) {
    ...
    if (rf95.recv(buf, &len)) {
      ...
      if (strstr((char *)buf, "in")) {
        ...
        //Handled by Payload Ematch radio
  ...
  if (gps_transmit == true && !rf95.available()) {
    ...
    if (millis() - timer > 2000) {
      timer = millis();
      if (GPS.fix) {
        delay(1000);  //Wait a second so we aren't spamming the ground radio
        char location[100];
        uint8_t hour = GPS.hour;  //hour
        uint8_t minute = GPS.minute;  //minute
        uint8_t second = GPS.seconds;  //seconds
        uint8_t milliseconds = GPS.milliseconds;  //milliseconds
        ...
        sprintf(location, "\nTime: %u:%u:%u.%u \nLocation: %f%c, %f%c\n", hour, minute, second, milliseconds, lat, latC, lon, lonC);  // Format outgoing GPS location
        rf95.send((uint8_t *)location, 100);
        rf95.waitPacketSent();
        Serial.print("Sent GPS location: "); Serial.println(location);
      }
      else {
        // Send fix error
        uint8_t location[] = "\nGPS error: unable to get a fix";
        delay(1000);  //Wait a second so we aren't spamming the ground radio
        rf95.send(location, sizeof(location));
        rf95.waitPacketSent();
        Serial.print("Sent GPS error: "); Serial.println((char *)location); 
      }
    }
  }
}
